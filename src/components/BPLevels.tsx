...
const BPLevelsChart = () => {
  ...
  const fetchPatientData = async () => {
    try {
      ...
      const systolicBP = healthMetrics.map((metric) => metric.blood_pressure.systolic);
      ...
      setChartData((prevState) => ({
        ...prevState,
        labels: dates,
        datasets: [{ ...prevState.datasets[0], data: systolicBP }],
      }));
          ...
          <tbody>
            {tableData.map((metric, index) => (
              <tr key={index} className="border-t">
                ...
                <td className="px-4 py-2">{metric.blood_pressure.systolic}</td>
              </tr>
            ))}
          </tbody>
        </table>
