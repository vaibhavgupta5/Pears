...
const BPLevelsChart = () => {
    ...
    datasets: [
      {
        ...
        borderColor: "rgba(34, 197, 94, 1)", // Tailwind's red-500
  ...
  const fetchPatientData = async () => {
    try {
      ...
      const Oxygen = healthMetrics.map((metric) => metric.oxygen_saturation);
  ...
  return (
    <div className="max-w-sm bg-white rounded-lg text-black p-4">
      ...
      <div className="text-center">
        <h2 className="text-lg font-semibold">Oxygen Levels</h2>
          ...
          <tbody>
            {tableData.map((metric, index) => (
              <tr key={index} className="border-t">
                ...
                <td className="px-4 py-2">{metric.oxygen_saturation}</td>
              </tr>
            ))}
          </tbody>
        </table>
